...
> = (state) => {
  ...
  const onCreateClick = () => {
    ...
    smartViews.saveCurrnetState(name);
  };
  ...
  return BUI.html`
    <bim-panel-section fixed icon=${appIcons.SMARTVIEW} label="Smart Views">
          ...
          <bim-button style="flex: 0" icon=${appIcons.ADD}>
            <bim-context-menu>
              <div style="display: flex; gap: 0.5rem;">
                ...
                <bim-button @click=${onCreateClick} style="width: 10rem" label="Create"></bim-button>
              </div>
            </bim-context-menu>
          </bim-button>
        </div>
