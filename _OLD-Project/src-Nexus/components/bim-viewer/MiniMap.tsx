...
/**
 * Top-down floor-plan minimap rendered into the SAME WebGL context as the
 * main viewer using scissor + viewport on a bottom-right inset.
 *
 * Because we use a non-zero useFrame priority, R3F hands the render loop
 * to us — so we must explicitly render the main scene FIRST (filling the
 * full canvas), then render the inset on top. This also guarantees the
 * inset is always square regardless of canvas aspect or panel collapses.
 */
export function MiniMap({ size = 200, padding = 16 }: { size?: number; padding?: number }) {
  ...
  // Decorative border + background pinned over the canvas corner.
  ...
  useFrame(({ clock }) => {
    // Compute expensive scene bounds at a modest cadence; camera marker still updates every frame.
    if (clock.elapsedTime >= boundsRef.current.nextUpdate) {
      const { box, center, size: sizeV } = boundsRef.current;
      box.makeEmpty();
      scene.traverse((obj) => {
        if (!obj.visible || obj.userData.bimSectionHelper) return;
    ...
    // Render the square inset in logical CSS pixels. Three.js multiplies by DPR internally;
    // passing physical pixels here corrupts the renderer viewport and breaks pointer alignment.
    ...
    // Restore for Drei HUD/viewcube and R3F pointer math.
