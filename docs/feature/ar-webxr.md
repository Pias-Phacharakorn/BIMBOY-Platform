...
- AR reuses the same fragments/model pipeline as the main viewer (`FragmentsManager`, same worker) — don't fork model loading, even though the live path runs an isolated `OBC.Components` instance disconnected from any `World`. ⚠️ Because that instance is its own, `useArModelLoader` carries **its own** `import fragmentsWorkerUrl from "@thatopen/fragments/worker?url"` and its own `fragments.init(...)`; it inherits nothing from `setupFragmentsManager`. Both call sites once hardcoded `"/worker.mjs"` and both had to change together — see [ADR-0014](../adr/0014-frags-worker-from-node-modules.md).
