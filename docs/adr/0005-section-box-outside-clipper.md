...
- **Six `GizmoAxis` handles in the existing `"axes"` form** (renamed `"plane"` by [ADR-0009](0009-section-plane-gizmo-local-frame.md); the reasoning below is unaffected) — zero engine change. Rejected on geometry: `buildAxisGizmo` draws all three axes double-headed with a centre diamond, so a box would carry six diamonds and eighteen arrows, and its picker is a cylinder *centred on the origin* — on a thin box the +X and −X pickers overlap through the middle, so the near face's handle grabs the far one.
