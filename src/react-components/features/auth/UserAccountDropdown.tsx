...
export function UserAccountDropdown() {
  ...
  const initials = profile?.email?.slice(0, 2).toUpperCase() || user?.email?.slice(0, 2).toUpperCase() || "US";
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2.5 w-60 p-1.5 rounded-radius border border-border bg-[oklch(14.5%_0.014_255_/_94%)] backdrop-blur-md shadow-2xl z-50 animate-in fade-in slide-in-from-top-2 duration-120">
          ...
          <div className="px-3.5 py-3 flex flex-col gap-1 select-none">
            <span className="text-xs text-muted-2 font-bold uppercase tracking-wider">Logged in as</span>
            <span className="text-sm font-semibold text-fg truncate" title={user.email}>
              {user.email}
            </span>
